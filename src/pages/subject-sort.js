...
for (let i = 4; i < uniJSON.topics.length; i++) {
  for (let j = 0; j < uniJSON.topics[i].programs.length; j++) {
    ...
    subjectArray.push(newEntry);
  }
}

//sort array
subjectArray.sort((a, b) => a.title.localeCompare(b.title));
...
const subjectSort = () => {
  ...
  subjectList.classList.add("a-z-div");

  //Set heading (h3)
  let subjectTitle = document.createElement("h3");
  subjectTitle.innerHTML = "Individual Subject events";
  subjectList.appendChild(subjectTitle);

  subjectArray.forEach((newEntry, k) => {
    ...
    subjectList.appendChild(entryDiv);
  });

  //append to main content
  mainContent.appendChild(subjectList);
};

export { subjectSort, subjectArray };
